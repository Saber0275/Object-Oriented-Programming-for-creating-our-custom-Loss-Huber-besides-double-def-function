...
#Child class & Parent class (inheritance)
class MyHuberLoss(Loss):  #Inherit from the aforementioned Keras ► MyHuberLoss is a Child class, and Loss is a Parent class
    def __init__(self, threshold=1): #setting the default value to 1
        super(MyHuberLoss, self).__init__() #cuz it is inheritance
        self.threshold = threshold #► attrbute  #we can use it further as hyperparameter (default=1)

    # compute loss
    def call(self, y_true, y_pred):
        error = y_true - y_pred #attribute
